...
但每次循环每个连接都要询问是否有数据这一个动作（很多是无意义的）
--------------------------------------------------------------------------
多路复用器 select, poll
通过多路复用器只能获取状态，然后由程序自己对着有状态的io进行读写
只要程序自己读写那么你的io模型就是同步的
只关注io:不关注io之后的事（从io读写完之后的事情）
同步：app自己R/W
异步：由kernel完成R/W
阻塞：BLOCKING 程序自己读取阻塞调用了方法一直等待有效返回结果
非阻塞：NONBLOCKING 程序自己读取调用了方法一瞬间给出是否读到（自己要解决下次啥时候再去读）
linux,以及成熟的框架netty，常用的就是同步阻塞和同步非阻塞
异步：尽量不要去讨论，因为linux目前没有通用的内核的异步处理方式

多路复用器是将文件描述符交给内核遍历
1.select(POSIX标准的)  1024
2.POLL()  没有1024的限制
3.EPOLL
无论nio select poll都要遍历所有的io,询问状态
NIO：这个遍历的成本在用户态和内核切换
select,poll： 这个遍历的过程出发了一次系统调用，用户态内核态的切换，在过程中，把fds传递给内核
内核重新根据用户这次调用传来的fds，遍历，修改状态

select,poll的弊端
1.每次都要重新传递fds
2.每次内核被调了之后针对这次调用触发一个遍历fds全量的复杂度

插入一个概念：
中断会产生回调    event时间->回调处理事件
在epoll之前的cb：只是完成了将网卡发来的数据，走内核网络协议栈(2链路层,3网络层,4传输控制层)最终关联到fd的buffer,所以你在某一个时间如果从app访问内核某一个或者多个fd是否可读可写，会有状态返回

EPOLL
epoll_create: 创建一个内存区域（红黑树）并返回一个描述此区域的文件描述符
epoll_ctl:通过此区域的文件描述符对此区域进行增加删除修改操作
epoll_wait:去链表中寻找是否有可读可写事件
epoll：对中断做了延伸在原来的基础上增加功能：在完成了关联df的buffer之后，在红黑树中查找数据包中解析出来的文件描述符是否在红黑树中，如果存在则将此文件描述符拷贝到链表中，供epoll_wait调用返回(减少了遍历fd的过程)
```
